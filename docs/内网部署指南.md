# Smart Water 内网部署指南

本文档详细说明如何通过 U 盘将项目从联网机器部署到内网机器。

---

## 📋 部署流程概览

```
┌─────────────────┐         U盘          ┌─────────────────┐
│   联网机器       │  ─────────────────▶  │   内网机器       │
│                 │                      │                 │
│  1. 拉取镜像     │   镜像 tar 文件      │  1. 加载镜像     │
│  2. 构建镜像     │   + 项目代码         │  2. 配置环境     │
│  3. 导出到 tar   │                      │  3. 启动服务     │
└─────────────────┘                      └─────────────────┘
```

---

## 第一部分：联网机器操作

### 1. 环境要求

- Windows 10/11 或 Linux
- Docker Desktop 已安装并运行
- 项目代码已克隆

### 2. 一键打包（推荐）

```powershell
# 进入项目目录

# 运行打包脚本
.\docker\save_docker_images.ps1
```

脚本会自动执行：
1. ✅ 拉取所有基础镜像
2. ✅ 构建 API 和 Web 镜像
3. ✅ 导出到 `docker_images_backup\` 目录
4. ✅ 生成部署清单

### 3. 如果网络不好

使用镜像加速拉取：

```bash
# 使用 DaoCloud 镜像加速（Linux/Git Bash）
./docker/pull_image.sh python:3.12-slim
./docker/pull_image.sh neo4j:5.26
./docker/pull_image.sh milvusdb/milvus:v2.5.6
```

### 4. 需要复制到 U 盘的文件

```
U盘/
├── docker_images_backup/
│   ├── smart_water_images_YYYYMMDD_HHMM.tar  # ~5-8 GB
│   └── 部署清单_YYYYMMDD_HHMM.txt
└── Yuxi-Know-main/                            # 项目代码（完整目录）
    ├── docker/
    ├── docker-compose.yml
    ├── server/
    ├── src/
    ├── web/
    ├── .env.template
    ├── deploy.ps1
    └── ...
```

> [!IMPORTANT]
> 确保 U 盘剩余空间 > 15 GB（FAT32 格式不支持大于 4GB 的单个文件，请使用 NTFS 或 exFAT 格式）

---

## 第二部分：内网机器操作

### 1. 环境准备

确保内网机器已安装：
- Docker Desktop（Windows）或 Docker Engine（Linux）
- Docker Compose V2

### 2. 复制文件

从 U 盘复制到内网机器：

```powershell
# 示例：复制到 D:\projects\
copy-item -Recurse E:\Yuxi-Know-main D:\projects\
copy-item -Recurse E:\docker_images_backup D:\projects\
```

### 3. 加载 Docker 镜像

```powershell
# 进入目录
cd D:\projects

# 加载镜像（约需 5-10 分钟）
docker load -i docker_images_backup\smart_water_images_*.tar

# 验证镜像已加载
docker images
```

预期输出：
```
REPOSITORY                     TAG                            SIZE
smart-water-api                1.0.0                          ~1.5 GB
smart-water-web                1.0.0                          ~500 MB
neo4j                          5.26                           ~600 MB
milvusdb/milvus                v2.5.6                         ~1.5 GB
minio/minio                    RELEASE.2023-03-20T20-16-18Z   ~200 MB
quay.io/coreos/etcd            v3.5.5                         ~50 MB
```

### 4. 配置环境变量

```powershell
cd D:\projects\Yuxi-Know-main

# 创建配置文件
copy .env.template .env

# 编辑配置
notepad .env
```

**必须配置的项目**：

```ini
# ========== LLM API 配置 ==========
# 方式一：使用内网 vLLM 服务
OPENAI_API_BASE=http://192.168.x.x:8000/v1
OPENAI_API_KEY=no-key-needed

# 方式二：使用硅基流动等云服务（如内网可访问）
SILICONFLOW_API_KEY=sk-xxxxx

# ========== 管理员账号 ==========
YUXI_SUPER_ADMIN_NAME=admin
YUXI_SUPER_ADMIN_PASSWORD=your_secure_password

# ========== 数据库密码 ==========
NEO4J_PASSWORD=0123456789
```

### 5. 启动服务

```powershell
# 使用部署脚本启动
.\deploy.ps1 start

# 或直接使用 docker compose
docker compose up -d
```

### 6. 检查服务状态

```powershell
# 查看所有服务状态
.\deploy.ps1 status

# 查看日志
.\deploy.ps1 logs
```

等待所有服务变为 `healthy`（约 3-5 分钟）。

### 7. 访问服务

| 服务 | 地址 | 用途 |
|------|------|------|
| **前端界面** | http://localhost:5173 | 用户访问入口 |
| **API 文档** | http://localhost:5050/docs | Swagger API 文档 |
| **Neo4j 浏览器** | http://localhost:7474 | 图数据库管理 |
| **MinIO 控制台** | http://localhost:9001 | 对象存储管理 |

---

## 第三部分：GPU 服务部署（MinerU / PaddleX OCR）

> [!WARNING]
> GPU 服务镜像需要在联网机器上**完整构建**，不能只下载基础镜像！
> 构建过程中会自动下载 AI 模型文件（约 5-10GB），这些模型会被打包到镜像中。

### 硬件要求

| 组件 | 最低要求 | 推荐配置 |
|------|---------|---------|
| GPU | NVIDIA GPU 8GB 显存 | 16GB+ 显存 |
| Driver | >= 525 | >= 535 |
| CUDA | 12.6 | 12.6 |
| 系统组件 | nvidia-container-toolkit | |

---

### MinerU OCR 服务

MinerU 是一个文档 OCR 服务，用于提取 PDF/图片中的文字。

#### 镜像说明

```dockerfile
# docker/mineru.Dockerfile 关键内容：
FROM lmsysorg/sglang:v0.4.9.post3-cu126     # 基础镜像 ~15GB
RUN pip install mineru[core]                 # 安装 Python 包
RUN mineru-models-download -s modelscope -m all  # 下载模型 ~5-10GB
```

| 阶段 | 镜像大小 | 说明 |
|------|---------|------|
| 基础镜像 | ~15 GB | `lmsysorg/sglang:v0.4.9.post3-cu126` |
| 构建后 | ~20-25 GB | 包含 MinerU + 所有模型 |

#### 联网机器：构建 MinerU 镜像

```powershell
cd d:\大坝最新项目\Yuxi-Know-main

# 步骤 1: 拉取基础镜像（约 15GB，耐心等待）
docker pull lmsysorg/sglang:v0.4.9.post3-cu126

# 步骤 2: 构建 MinerU 镜像（会下载模型，约 30-60 分钟）
docker compose build mineru

# 步骤 3: 验证镜像已构建
docker images | findstr mineru
# 预期输出：mineru-sglang   latest   xxxxx   20-25GB

# 步骤 4: 导出镜像
docker save mineru-sglang:latest -o docker_images_backup/mineru_ocr.tar
```

> [!TIP]
> 如果基础镜像下载慢，可使用镜像加速：
> ```bash
> ./docker/pull_image.sh lmsysorg/sglang:v0.4.9.post3-cu126
> ```

#### 内网机器：加载并启动

```powershell
# 加载镜像（约 10-15 分钟）
docker load -i docker_images_backup/mineru_ocr.tar

# 启动包含 GPU 服务的完整环境
.\deploy.ps1 start-all

# 或只启动 MinerU 服务
docker compose --profile all up -d mineru
```

#### 验证 MinerU 服务

```powershell
# 检查服务状态
docker compose ps mineru

# 检查健康状态
curl http://localhost:30000/health
```

---

### PaddleX OCR 服务

PaddleX 是百度的文档结构识别服务，支持表格、公式、印章等识别。

#### 镜像说明

```dockerfile
# docker/paddlex.Dockerfile 关键内容：
FROM ccr-2vdh3abv-pub.cnc.bj.baidubce.com/paddlex/paddlex:paddlex3.0.1-...
RUN paddlex --install hpi-cpu
RUN paddlex --install serving
```

| 阶段 | 镜像大小 | 说明 |
|------|---------|------|
| 基础镜像 | ~20 GB | 百度云镜像 |
| 构建后 | ~22 GB | 包含 PaddleX + 配置 |

#### 联网机器：构建 PaddleX 镜像

```powershell
# 步骤 1: 拉取基础镜像（百度云，国内速度较快）
docker pull ccr-2vdh3abv-pub.cnc.bj.baidubce.com/paddlex/paddlex:paddlex3.0.1-paddlepaddle3.0.0-gpu-cuda11.8-cudnn8.9-trt8.6

# 步骤 2: 构建 PaddleX 镜像
docker compose build paddlex

# 步骤 3: 导出镜像
docker save paddlex:latest -o docker_images_backup/paddlex_ocr.tar
```

#### 内网机器：加载并启动

```powershell
docker load -i docker_images_backup/paddlex_ocr.tar
docker compose --profile all up -d paddlex
```

---

### GPU 服务一键打包

使用打包脚本自动处理：

```powershell
# 联网机器：构建并导出所有镜像（包括 GPU 服务）
.\docker\save_docker_images.ps1 -IncludeGpu

# 生成的文件：
# - docker_images_backup/smart_water_images_*.tar      (~5-8 GB)
# - docker_images_backup/smart_water_gpu_images_*.tar  (~40-50 GB)
```

### U 盘格式要求

因为 GPU 镜像文件很大，**必须使用 NTFS 或 exFAT 格式**：

```powershell
# 检查 U 盘格式
Get-Volume -DriveLetter E | Select-Object FileSystemType

# 如果是 FAT32，需要重新格式化为 NTFS
# 警告：会清除所有数据！
Format-Volume -DriveLetter E -FileSystem NTFS -Confirm
```

---

### 常见问题

#### Q: 为什么不能只下载基础镜像？

A: MinerU 的 Dockerfile 中有 `mineru-models-download` 步骤，这个命令会在构建时从 ModelScope 下载约 5-10GB 的 AI 模型。如果只下载基础镜像，内网构建时无法下载模型会导致失败。

#### Q: GPU 镜像太大怎么办？

A: 可以考虑以下方案：
1. 分卷压缩：使用 7-Zip 将 tar 文件分割成多个 4GB 以下的文件
2. 使用移动硬盘：比 U 盘传输更快
3. 内网搭建临时 Docker Registry：一次传输，多机使用

#### Q: 没有 GPU 能用 OCR 吗？

A: 不能在本地运行 MinerU/PaddleX，但可以：
1. 使用外部 OCR API 服务
2. 在有 GPU 的服务器上部署 OCR 服务，API 方式调用

---

## 故障排除

### 问题：镜像加载失败

```powershell
# 检查 tar 文件完整性
tar -tf smart_water_images_*.tar | Select-Object -First 10

# 如文件损坏，需重新从联网机器复制
```

### 问题：服务启动失败

```powershell
# 查看详细错误日志
docker compose logs api
docker compose logs milvus

# 检查端口是否被占用
netstat -ano | findstr "5050"
netstat -ano | findstr "5173"
```

### 问题：API 无法连接 LLM

确保 `.env` 中的 LLM 配置正确：

```ini
# 检查服务是否可达
curl http://192.168.x.x:8000/v1/models
```

### 问题：前端无法访问后端

检查 Docker 网络：

```powershell
# 确保容器在同一网络
docker network ls
docker network inspect yuxi-know-main_app-network
```

---

## 附录：镜像清单

| 镜像 | 版本 | 大小 | 用途 |
|------|------|------|------|
| `smart-water-api` | 1.0.0 | ~1.5GB | 后端 API |
| `smart-water-web` | 1.0.0 | ~500MB | 前端服务 |
| `neo4j` | 5.26 | ~600MB | 图数据库 |
| `milvusdb/milvus` | v2.5.6 | ~1.5GB | 向量数据库 |
| `minio/minio` | RELEASE.2023-03-20T20-16-18Z | ~200MB | 对象存储 |
| `quay.io/coreos/etcd` | v3.5.5 | ~50MB | Milvus 依赖 |

---

## 附录：文件说明

| 文件/目录 | 说明 |
|----------|------|
| `docker-compose.yml` | 服务编排配置 |
| `docker/api.Dockerfile` | API 镜像构建文件 |
| `docker/web.Dockerfile` | Web 镜像构建文件 |
| `docker/save_docker_images.ps1` | 镜像打包脚本 |
| `docker/pull_image.sh` | 镜像加速拉取脚本 |
| `docker/nginx/` | Nginx 配置文件 |
| `docker/volumes/` | 数据持久化目录 |
| `.env` | 环境变量配置 |
| `deploy.ps1` | 部署管理脚本 |

---

## 附录：数据备份与恢复

### 数据存储位置

所有持久化数据存储在 `docker/volumes/` 目录下：

```
docker/volumes/
├── neo4j/
│   ├── data/          # Neo4j 图数据库数据
│   └── logs/          # Neo4j 日志
├── milvus/
│   ├── milvus/        # Milvus 向量数据
│   ├── etcd/          # Milvus 元数据
│   ├── minio/         # MinIO 对象存储数据
│   └── logs/          # Milvus 日志
└── paddlex/           # PaddleX 数据
```

### 备份数据

```powershell
# 停止服务（确保数据一致性）
.\deploy.ps1 stop

# 备份整个 volumes 目录
$BackupDate = Get-Date -Format "yyyyMMdd"
Compress-Archive -Path "docker/volumes" -DestinationPath "backup_$BackupDate.zip"

# 重新启动服务
.\deploy.ps1 start
```

### 备份单个服务

```powershell
# 只备份 Neo4j 数据
docker run --rm -v yuxi-know-main_neo4j-data:/data -v ${PWD}:/backup alpine tar czf /backup/neo4j_backup.tar.gz /data

# 只备份 Milvus 数据
Compress-Archive -Path "docker/volumes/milvus" -DestinationPath "milvus_backup.zip"
```

### 恢复数据

```powershell
# 停止服务
.\deploy.ps1 stop

# 删除旧数据（谨慎操作！）
Remove-Item -Recurse -Force "docker/volumes"

# 解压备份数据
Expand-Archive -Path "backup_YYYYMMDD.zip" -DestinationPath "docker/"

# 重新启动服务
.\deploy.ps1 start
```

### 迁移到新机器

```powershell
# 在旧机器上
.\deploy.ps1 stop
Compress-Archive -Path "docker/volumes" -DestinationPath "data_migration.zip"

# 复制到新机器后
Expand-Archive -Path "data_migration.zip" -DestinationPath "docker/"
.\deploy.ps1 start
```

---

## 附录：GPU 服务依赖离线安装

在内网机器上运行 GPU 服务需要安装 NVIDIA Container Toolkit。

### Windows 环境

Windows 使用 Docker Desktop，GPU 支持通过 WSL2 实现：

1. **安装 NVIDIA GPU 驱动**（必须联网下载后离线安装）
   - 从 [NVIDIA 官网](https://www.nvidia.com/drivers) 下载对应型号的驱动
   - 复制到内网机器安装

2. **配置 Docker Desktop**
   - 确保 Docker Desktop 使用 WSL2 后端
   - Settings → Resources → WSL Integration → 启用

3. **验证 GPU 可用**
   ```powershell
   docker run --rm --gpus all nvidia/cuda:12.0-base nvidia-smi
   ```

### Linux 环境 (Ubuntu/Debian)

#### 步骤 1：下载离线安装包（在联网机器上）

```bash
# 创建下载目录
mkdir -p nvidia_offline && cd nvidia_offline

# 下载 NVIDIA 驱动（根据 GPU 型号选择）
wget https://us.download.nvidia.com/XFree86/Linux-x86_64/535.129.03/NVIDIA-Linux-x86_64-535.129.03.run

# 下载 nvidia-container-toolkit
# 添加仓库并下载包
curl -fsSL https://nvidia.github.io/libnvidia-container/gpgkey | sudo gpg --dearmor -o /usr/share/keyrings/nvidia-container-toolkit-keyring.gpg

# 下载所需的 deb 包
apt-get download nvidia-container-toolkit nvidia-container-toolkit-base libnvidia-container1 libnvidia-container-tools

# 打包所有文件
cd ..
tar czf nvidia_offline.tar.gz nvidia_offline/
```

#### 步骤 2：离线安装（在内网机器上）

```bash
# 解压
tar xzf nvidia_offline.tar.gz
cd nvidia_offline

# 安装 NVIDIA 驱动
chmod +x NVIDIA-Linux-x86_64-*.run
sudo ./NVIDIA-Linux-x86_64-*.run --silent

# 安装 nvidia-container-toolkit
sudo dpkg -i *.deb

# 配置 Docker 使用 NVIDIA runtime
sudo nvidia-ctk runtime configure --runtime=docker
sudo systemctl restart docker

# 验证
docker run --rm --gpus all nvidia/cuda:12.0-base nvidia-smi
```

### CentOS/RHEL 环境

```bash
# 在联网机器上下载 RPM 包
yumdownloader nvidia-container-toolkit --resolve --destdir=./nvidia_rpms

# 在内网机器上安装
sudo rpm -ivh nvidia_rpms/*.rpm
```

---

## 附录：Linux 版本部署说明

### 环境要求

- Ubuntu 20.04+ / Debian 11+ / CentOS 8+
- Docker Engine 20.10+
- Docker Compose V2 (docker compose 命令)

### 一键打包（联网机器）

```bash
cd /path/to/Yuxi-Know-main

# 运行打包脚本
chmod +x docker/save_docker_images.sh
./docker/save_docker_images.sh
```

### 部署步骤（内网机器）

#### 1. 安装 Docker（离线）

```bash
# 在联网机器上下载 Docker 离线包
# 访问 https://download.docker.com/linux/static/stable/x86_64/
# 下载 docker-24.0.7.tgz

# 在内网机器上安装
tar xzf docker-24.0.7.tgz
sudo cp docker/* /usr/bin/

# 创建 systemd 服务
sudo tee /etc/systemd/system/docker.service << 'EOF'
[Unit]
Description=Docker Application Container Engine
After=network-online.target

[Service]
Type=notify
ExecStart=/usr/bin/dockerd
Restart=always

[Install]
WantedBy=multi-user.target
EOF

sudo systemctl daemon-reload
sudo systemctl enable docker
sudo systemctl start docker
```

#### 2. 加载镜像

```bash
cd /path/to/project

# 加载镜像
docker load -i docker_images_backup/smart_water_images_*.tar

# 验证
docker images
```

#### 3. 配置环境

```bash
cd Yuxi-Know-main

# 创建配置文件
cp .env.template .env

# 编辑配置
nano .env  # 或 vim .env
```

#### 4. 启动服务

```bash
# 使用部署脚本
chmod +x deploy.sh
./deploy.sh start

# 或直接使用 docker compose
docker compose up -d

# 查看状态
docker compose ps
```

#### 5. 配置防火墙

```bash
# Ubuntu/Debian (ufw)
sudo ufw allow 5173/tcp  # 前端
sudo ufw allow 5050/tcp  # API
sudo ufw allow 7474/tcp  # Neo4j
sudo ufw allow 9001/tcp  # MinIO

# CentOS/RHEL (firewalld)
sudo firewall-cmd --permanent --add-port=5173/tcp
sudo firewall-cmd --permanent --add-port=5050/tcp
sudo firewall-cmd --permanent --add-port=7474/tcp
sudo firewall-cmd --permanent --add-port=9001/tcp
sudo firewall-cmd --reload
```

### Linux 常用命令对照表

| 操作 | Windows (PowerShell) | Linux (Bash) |
|------|---------------------|--------------|
| 启动服务 | `.\deploy.ps1 start` | `./deploy.sh start` |
| 停止服务 | `.\deploy.ps1 stop` | `./deploy.sh stop` |
| 查看状态 | `.\deploy.ps1 status` | `./deploy.sh status` |
| 查看日志 | `.\deploy.ps1 logs` | `./deploy.sh logs` |
| 复制文件 | `copy-item` | `cp` |
| 压缩文件 | `Compress-Archive` | `tar czf` |
| 解压文件 | `Expand-Archive` | `tar xzf` |
| 编辑文件 | `notepad` | `nano` / `vim` |

### Linux 专有问题

#### 权限问题

```bash
# 将当前用户添加到 docker 组
sudo usermod -aG docker $USER

# 重新登录或执行
newgrp docker
```

#### volumes 目录权限

```bash
# 如果遇到权限问题
sudo chown -R 1000:1000 docker/volumes/neo4j
sudo chown -R 1000:1000 docker/volumes/milvus
```

#### SELinux 问题（CentOS/RHEL）

```bash
# 临时禁用
sudo setenforce 0

# 或添加 Docker 卷标签
docker run -v /path:/container:Z ...
```
